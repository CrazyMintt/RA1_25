...
class geradorAssembly():
    ...
    def liberar_reg(self, reg: str):
        ...
        if reg.startswith("D"):
            ...
            if reg not in self.regs_livres_int:
    ...
    def resolver_parenteses(self, tokens: list[Token]):
        ...
        while i < len(tokens):
            ...
            if token.tipo == TipoToken.PARENTESE_ESQ:
                ...
                pilha_resultado = self._gerar_linha_assembly(tokens_in_p)
                
                # O resultado do parêntese deve ser o que sobrou na pilha
                tokens[i_abertura: i + 1] = pilha_resultado
                i = i_abertura + len(pilha_resultado)
            else:
                i += 1
        return tokens
    ...
    def _gerar_linha_assembly(self, tokens_line):
        ...
        for token in tokens_line:
            # Caso seja uma variável (ex: res_linha_0) injetada pelo resolver_RES
        ...
        for i, tokens_line in enumerate(token_matrix):
            for j, token in enumerate(tokens_line):
                if token.tipo == TipoToken.KEYWORD and token.valor == "RES":
                    # Pega o token anterior que indica o número da linha
                    valor_anterior = tokens_line[j-1]
                    linha_para_salvar = int(valor_anterior.valor)
                    nome_variavel = f"res_linha_{linha_para_salvar}"
                    
                    if nome_variavel not in self.memoria:
                        self.memoria[nome_variavel] = None 
                        # Adiciona a marcação de salvamento na linha de origem
    ...
    def gerarAssembly(self, token_matrix: list[list[Token]]) -> tuple[str, list]:
        # Reinicia os contadores para evitar lixo de chamadas anteriores
        self.codigo_assembly = []
        self.data_section = [] 
        self.label_display_counter = 0 

        # Gera os dados base (tabela 7seg e dez_float) antes de processar as linhas
        ...
        for i, tokens_line in enumerate(token_matrix):
            ...
            # Se a linha resultou em algo, mostramos no display e limpamos a pilha
            while pilha_final: 
                ...
                if reg_res:
                    ...
                    # LIBERA o registrador se for temporário
    ...
    def create_op_headers(self, a: Token, b: Token, operacao: Token):
        ...
        # Se a operação exige float mas um dos regs é int, converte dinamicamente
        if eh_float:
    ...
    def _criar_pow_loop(self, kw, reg_a, reg_b, reg_resultado, eh_float):
        ...
        if reg_b.startswith("D"):
            ...
            # Como reg_b era um D temporário ou de memória, 
            # não o liberamos aqui para não quebrar a lógica do chamador,
            # mas usaremos o reg_tmp_int para o controle do loop.
        ...
        before_op.append(f"    CMP {reg_contador}, #0") # Agora sempre usa um Reg R
        before_op.append(f"    BEQ {lbl_end}")
        before_op.append(f"    {kw} {reg_resultado}, {reg_resultado}, {reg_a}")
        before_op.append(f"    SUB {reg_contador}, {reg_contador}, #1") # Agora sempre usa um Reg R
        before_op.append(f"    B {lbl_loop}")
        before_op.append(f"{lbl_end}:")

        # Se alocamos um registrador inteiro extra para o contador, liberamos agora
...
#Estou chamando o que esta daqui pra trás de bilblioteca, são todos os códigos que geram as funções de display, divisão, etc. O que esta antes disso é o código de geração do assembly em si. Assim fica mais organizado e fácil de entender a parte principal do gerador.
    ...
    def gerar_data_section(self):
        ...
        self.data_section.append(".align 2") # Garante início alinhado
        self.data_section.append("tabela_7seg:")
        for num, hexa in mapa_7seg.items():
            self.data_section.append(f"    .byte {hexa}  @ dígito {num}")
            
        self.data_section.append(".align 3") # Alinha para o double (8 bytes)
        self.data_section.append("dez_float: .double 10.00")

    def _criar_label_float(self, valor) -> str:
        label = f"num_{self.data_counter}"
        self.data_counter += 1
        # Sempre alinha antes de um novo double para evitar o erro de desalinhamento
